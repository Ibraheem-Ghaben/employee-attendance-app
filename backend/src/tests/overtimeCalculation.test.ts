...
import { EmployeePayConfig, PunchSpan } from '../types/overtime';

describe('OvertimeCalculationService', () => {
  // Sample configuration
  const baseConfig: EmployeePayConfig = {
  ...
  describe('calculateRates', () => {
    it('should calculate rates with multipliers correctly', () => {
      const rates = overtimeCalculationService.calculateRates(baseConfig);
  ...
  describe('isWeekendDay', () => {
    it('should identify Friday as weekend', () => {
      ...
      const isWeekend = overtimeCalculationService.isWeekendDay(friday, 'Friday,Saturday');
      expect(isWeekend).toBe(true);
    });

    it('should identify Saturday as weekend', () => {
      const saturday = new Date('2025-01-04'); // Saturday
      const isWeekend = overtimeCalculationService.isWeekendDay(saturday, 'Friday,Saturday');
      expect(isWeekend).toBe(true);
    });

    it('should identify Monday as not weekend', () => {
      const monday = new Date('2025-01-06'); // Monday
      const isWeekend = overtimeCalculationService.isWeekendDay(monday, 'Friday,Saturday');
  ...
  describe('getDayName', () => {
    it('should return correct day names', () => {
      expect(overtimeCalculationService.getDayName(new Date('2025-01-05'))).toBe('Sunday');
      expect(overtimeCalculationService.getDayName(new Date('2025-01-06'))).toBe('Monday');
      expect(overtimeCalculationService.getDayName(new Date('2025-01-07'))).toBe('Tuesday');
      expect(overtimeCalculationService.getDayName(new Date('2025-01-03'))).toBe('Friday');
      expect(overtimeCalculationService.getDayName(new Date('2025-01-04'))).toBe('Saturday');
    });
  });
  ...
  describe('minutesOverlap', () => {
    it('should calculate overlap correctly', () => {
      ...
      const overlap = overtimeCalculationService.minutesOverlap(span, range);
  ...
  describe('pairPunchesToSpans', () => {
    it('should pair IN/OUT punches correctly', () => {
      const punches = [
        { punch_time: new Date('2025-01-06T09:00:00'), punch_type: 'IN' as const },
        { punch_time: new Date('2025-01-06T17:00:00'), punch_type: 'OUT' as const },
      ];
    ...
    it('should handle multiple pairs', () => {
      const punches = [
        { punch_time: new Date('2025-01-06T09:00:00'), punch_type: 'IN' as const },
        { punch_time: new Date('2025-01-06T12:00:00'), punch_type: 'OUT' as const },
        { punch_time: new Date('2025-01-06T13:00:00'), punch_type: 'IN' as const },
        { punch_time: new Date('2025-01-06T17:00:00'), punch_type: 'OUT' as const },
      ];
  ...
  describe('calculate - Workday Scenarios', () => {
    it('should calculate regular hours only (no overtime)', () => {
      ...
      const result = overtimeCalculationService.calculate(date, spans, baseConfig);
  ...
  describe('validateConfig', () => {
    it('should validate correct configuration', () => {
      const validation = overtimeCalculationService.validateConfig(baseConfig);
  ...
  describe('getWeekStart', () => {
    it('should get week start for Sunday start', () => {
      ...
      const weekStart = overtimeCalculationService.getWeekStart(date, 'Sunday');
    ...
    it('should get week start for Monday start', () => {
      const date = new Date('2025-01-08'); // Wednesday
      const weekStart = overtimeCalculationService.getWeekStart(date, 'Monday');
  ...
  describe('roundToCents', () => {
    it('should round to 2 decimal places', () => {
      expect(overtimeCalculationService.roundToCents(10.126)).toBe(10.13);
      expect(overtimeCalculationService.roundToCents(10.124)).toBe(10.12);
      expect(overtimeCalculationService.roundToCents(10.125)).toBe(10.13);
    });
  });
  ...
  describe('Complex Scenario: Full Week', () => {
    it('should calculate mixed week correctly', () => {
      ...
      const mondaySpans: PunchSpan[] = [{
        punch_in: new Date('2025-01-06T09:00:00'),
        punch_out: new Date('2025-01-06T18:00:00'),
        duration_minutes: 540,
      }];
      const mondayResult = overtimeCalculationService.calculate(monday, mondaySpans, baseConfig);
      ...
      const fridaySpans: PunchSpan[] = [{
        punch_in: new Date('2025-01-10T10:00:00'),
        punch_out: new Date('2025-01-10T16:00:00'),
        duration_minutes: 360,
      }];
      const fridayResult = overtimeCalculationService.calculate(friday, fridaySpans, baseConfig);
