...
# 🔍 Employee Attendance System - MSS-HR.com Production Status Check
# Domain: MSS-HR.com
# Version: 3.0.0
# Author: MSS Software Team

echo "🔍 Employee Attendance System - MSS-HR.com Production Status Check"
echo "===================================================================="
echo "📅 Date: $(date)"
...
if ping -c 1 MSS-HR.com &> /dev/null; then
    print_success "MSS-HR.com is accessible"
else
    print_error "MSS-HR.com is not accessible"
fi

if ping -c 1 www.MSS-HR.com &> /dev/null; then
    print_success "www.MSS-HR.com is accessible"
else
    print_warning "www.MSS-HR.com is not accessible"
fi
echo ""
...
if [ -f "/etc/letsencrypt/live/MSS-HR.com/fullchain.pem" ]; then
    print_success "SSL certificate found for MSS-HR.com"
    
    # Check certificate expiry
    CERT_EXPIRY=$(openssl x509 -enddate -noout -in /etc/letsencrypt/live/MSS-HR.com/fullchain.pem | cut -d= -f2)
    echo "   Certificate expires: $CERT_EXPIRY"
    
    # Check if certificate is valid
    if openssl x509 -checkend 86400 -noout -in /etc/letsencrypt/live/MSS-HR.com/fullchain.pem; then
        print_success "SSL certificate is valid for at least 24 hours"
    else
        print_warning "SSL certificate expires within 24 hours"
    fi
else
    print_warning "SSL certificate not found for MSS-HR.com"
fi
echo ""
...
echo "   Frontend (HTTP):"
if curl -s -o /dev/null -w "%{http_code}" http://MSS-HR.com/ | grep -q "200"; then
...
echo "   Frontend (HTTPS):"
if curl -s -o /dev/null -w "%{http_code}" https://MSS-HR.com/ | grep -q "200"; then
    print_success "Frontend is accessible via HTTPS"
else
    print_warning "Frontend is not accessible via HTTPS"
fi

echo "   Backend API (HTTP):"
if curl -s -o /dev/null -w "%{http_code}" http://MSS-HR.com/api/ | grep -q "200\|404"; then
    print_success "Backend API is accessible via HTTP"
else
    print_error "Backend API is not accessible via HTTP"
fi

echo "   Backend API (HTTPS):"
if curl -s -o /dev/null -w "%{http_code}" https://MSS-HR.com/api/ | grep -q "200\|404"; then
    print_success "Backend API is accessible via HTTPS"
else
    print_warning "Backend API is not accessible via HTTPS"
fi
echo ""
...
echo ""
echo "✅ MSS-HR.com status check completed!"
